...
const removeItem = (index: number) => {
  ...
  else message.warning('单选题至少有两个选项')
}
    ...
    <n-grid :x-gap="6" :cols="3" @mouseenter="handleShow" @mouseleave="state.handle = false">
      <n-grid-item :span="2">
        ...
        <n-radio-group v-model:value="value" :name="qsId">
          <n-space>
            <n-radio v-for="option in qsData.qsOptions" :key="option.option_id" class="option" :value="option.option_id">
              {{ option.content }}
            </n-radio>
          </n-space>
        </n-radio-group>
...
// :v-deep(.n-form-item.n-form-item--top-labelled .n-form-item-label) {
//   align-items: center !important;
// }
// :v-deep(.n-form-item.n-form-item--left-labelled .n-form-item-label .n-form-item-label__asterisk) {
//   align-self: center;
// }
</style>
